...
1. **Artifacts (files)** — open the finished run → scroll to the bottom → **Artifacts** → download `kane-flow-output.zip`. Inside: the run log, every authored `_test.md`, (source mode only) `coverage.json`, and (source mode only) `traceability.html`.
...
## One dashboard for everything — pin a project

By default, kane-cli auto-picks a project for you, which means tests from different runs can scatter across different auto-created projects. To get **one single page showing every test's pass/fail history**, always pass the same `--project` name (locally) or fill the `project` field the same way every time (in GitHub Actions):

```
project: SauceDemo Suite
```

The first run creates that project in Test Manager; every run after reuses it (this repo's `resolve_or_create` logic checks by name first). From then on, `https://test-manager.lambdatest.com/projects/<that-project-id>` is your one bookmarkable dashboard — every test ever authored or replayed under that name, with pass/fail status and run history, in one place. Add a `folder` name too if you want to further group by feature area.

## Where to see the traceability report

Traceability (which requirement line is proven by which test) only exists for **source mode** (a real requirement doc was ingested — Quick Run and objective mode have no requirement to trace back to). Three ways to read it:

- **`traceability.html`** in the Artifacts zip — a self-contained visual graph (requirement → use-case → test), generated by `kane-cli context view`. Download it and open it in any browser, no server needed.
...
| `traceability.html` | local dir, or Artifacts zip | Source mode only — visual requirement→use-case→test graph, open in any browser |
| `.context/` | local dir only (gitignore this) | Source mode only — kane-cli's own store; never hand-edit |
| Test Manager dashboard (`share_url` in the log, or `test-manager.lambdatest.com/projects/<id>` if you pinned a `project`) | cloud, always | One place with every test's pass/fail history — see "One dashboard for everything" above |
